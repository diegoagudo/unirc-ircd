...
m_whois(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	...
	{
		sendto_one(source_p, form_str(ERR_NONICKNAMEGIVEN), MENAME(source_p), source_p->name);
		return;
	}
	...
	if(parc > 2 && !EmptyString(parv[2]))
	{
		...
		{
			sendto_one(source_p, form_str(RPL_LOAD2HI), MENAME(source_p), source_p->name);
			return;
		}
		else
	...
	if(!found)
	{
		if(!IsDigit(*nick))
			sendto_one(source_p, form_str(ERR_NOSUCHNICK),
				   MENAME(source_p), source_p->name, nick);
	}

	sendto_one(source_p, form_str(RPL_ENDOFWHOIS), MENAME(source_p), source_p->name, parv[1]);
}
...
whois_person(struct Client *source_p, struct Client *target_p)
{
    ...
    #if 0
    if(IsSOper(source_p) || !ConfigServerHide.hide_servers || target_p == source_p)
    ...
    if(target_p->vlink_nome) {
        sendto_one(source_p, form_str(RPL_WHOISSERVER), me.name, source_p->name, target_p->name, target_p->vlink_nome, target_p->vlink_desc ? target_p->vlink_desc : VlinkGetDesc(target_p->vlink_nome));
    } else {
        sendto_one(source_p, form_str(RPL_WHOISSERVER), me.name, source_p->name, target_p->name, server_p->name, server_p->info);
    }
    ...
    if(IsOper(target_p))
    {
        ...
        if(IsNetAdmin(target_p))
            strcat(buf, " - Services Root");
        else if(IsSAdmin(target_p))
        ...
        if(target_p->vlink_nome) {
            if(match(source_p->vlink_nome, target_p->vlink_nome)) {
                if(IsAnVOper(target_p))
                {
                    ...
                    if(buf[0]) {
                        sendto_one(source_p, form_str(RPL_WHOISOPERATOR), me.name, source_p->name, target_p->name, buf);
                    }
                }
            }
        }
    ...
    {  
        sendto_one(source_p, form_str(RPL_WHOISOPERATOR),
              me.name, source_p->name, target_p->name, "is an IRC helper");
    }

    if(IsSOper(source_p) && IsCaptured(target_p))
    ...
    if ( target_p == source_p || IsOper(source_p) || IsNetAdmin(source_p))
    {
        if (MyConnect(target_p) && IsSOper(source_p))
