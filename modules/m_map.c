...
struct Message map_msgtab = {
	...
	{m_unregistered, m_map, m_map, m_ignore, mo_map, m_ignore}
};
...
m_map(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	...
	if(!ConfigServerHide.flatten_links)
	{
        ...
        if(IsNetAdmin(source_p))
		{
            ...
            sendto_one(client_p, form_str(RPL_MAPEND), me.name, client_p->name, "MAP");
            return;
		} else  if(IsSAdmin(source_p)){
            dump_map(client_p, &me, 0, buf);
            sendto_one(client_p, form_str(RPL_MAPEND), me.name, client_p->name, "MAP");
            return;
        }
