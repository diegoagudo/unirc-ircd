...
m_join(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	...
	{
		sendto_one(source_p, form_str(ERR_NEEDMOREPARAMS), MENAME(source_p), source_p->name, "JOIN");
		return;
	}


    if ((IsGringo(source_p)) && (MyClient(source_p))) {
        if(!(match(CANAL_AJUDA, parv[1]))) {
	...
	for(chan = strtoken(&p, chan_list, ","); chan; chan = strtoken(&p, NULL, ","))
	{
		   ...
		   (!(resv_p = hash_find_resv(chan)) == ConfigChannel.restrict_channels))
		{
			if(resv_p != NULL)
				...
				sendto_one(source_p, form_str(ERR_CHANBANREASON),
					   MENAME(source_p), source_p->name, chan, "Reserved Channel");
		...
		if((i = can_join(source_p, chptr, key)))
		{
			if(i == ERR_NEEDREGGEDNICK)
				sendto_one(source_p, form_str(ERR_NEEDREGGEDNICK), MENAME(source_p),
			...
			/* we use ERR_CHANBANREASON for the SSL response due to wider client support */
				...
				sendto_one(source_p, form_str(ERR_CHANBANREASON), MENAME(source_p),
					   source_p->name, chptr->chname, "SSL is required");
			else
				sendto_one(source_p, form_str(i), MENAME(source_p), source_p->name,
					   chptr->chname);
			continue;
		}
		...
		if(flags & CHFL_CHANOP)
		{
			...
			sendto_server(client_p, source_p, chptr, NOCAPS, CAP_TS6, LL_ICLIENT,
				      ":%s SJOIN %lu %s +nt :@%s",
				      MENAME(source_p), (unsigned long) chptr->channelts,
		...
		if(chptr->topic != NULL)
		{
			sendto_one(source_p, form_str(RPL_TOPIC), MENAME(source_p),
				   source_p->name, chptr->chname, chptr->topic);

			sendto_one(source_p, form_str(RPL_TOPICWHOTIME),
				   MENAME(source_p), source_p->name, chptr->chname,
		...
		if(chptr->mode.forward[0]) {
			char* ajc = strdup(chptr->mode.forward); 
			parv[1] = source_p->name;
			parv[2] = ajc;
			m_join(source_p, source_p, 2, &parv[1]);
			free(ajc);

			sendto_one(source_p, ":%s NOTICE %s :*** Notice -- You are being forwarded %s",
					   me.name, source_p->name, chptr->mode.forward, source_p->name);
		}
	}
}
...
set_final_mode(struct Mode *mode, struct Mode *oldmode)
{
	...
	if(oldmode->forward[0] && !mode->forward[0])
	{
		...
		*mbuf++ = 'F';
		len = ircsprintf(pbuf, "%s ", oldmode->forward);
		pbuf += len;
	}
	...
	if(count != 0)
	{
		*mbuf = '\0';
		...
		sendto_channel_local(ALL_MEMBERS, NO, chptr,
				     ...
				     MENAME(source_p) : source_p->name, chptr->chname, lmodebuf, sendbuf);
	}
}
