...
struct Message kill_msgtab = {
	...
	{m_unregistered, ms_kill, ms_kill, m_ignore, mo_kill, m_ignore}
};
...
mo_kill(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
    ...
    if(*user == '\0')
    {
        sendto_one(source_p, form_str(ERR_NEEDMOREPARAMS), MENAME(source_p), source_p->name, "KILL");
        return;
    }
    ...
    if(!IsAnVOper(source_p) && !IsServer(source_p) && !IsServices(source_p)) {
        if(!IsOperK(source_p) && !IsOperGlobalKill(source_p))
        {
            sendto_one(source_p, form_str(ERR_NOPRIVILEGES), MENAME(source_p), source_p->name);
            return;
        }
    }
    ...
    if((target_p = find_client(user)) == NULL)
    {
                       ...
                       (time_t) ConfigFileEntry.kill_chase_time_limit)) == NULL)
        {
            sendto_one(source_p, form_str(ERR_NOSUCHNICK),
                   MENAME(source_p), source_p->name, user);
            return;
        }

        sendto_one(source_p, ":%s NOTICE %s :KILL changed from %s to %s",
               MENAME(source_p), source_p->name, user, target_p->name);
    }

    if(IsServer(target_p) || IsMe(target_p) || IsServices(target_p) || IsNetAdmin(target_p))
    {
        sendto_one(source_p, form_str(ERR_CANTKILLSERVER), MENAME(source_p), source_p->name);
        return;
    }

    /*
     * VOPER dando killlll... rs
     */
    if(!IsAnVOper(source_p)) {
        if(!MyConnect(target_p) && (!IsOperGlobalKill(source_p)))
        {
            sendto_one(source_p, ":%s NOTICE %s :Nick %s isnt on your server",
                   MENAME(source_p), source_p->name, target_p->name);
            return;
        }
    } else {
        if(source_p->vlink_nome) {
            if(target_p->vlink_nome) {
                if(!match(source_p->vlink_nome, target_p->vlink_nome)) {
                    ...
                    if(IsVAdmin(target_p) && target_p != source_p) {
                        sendto_one(source_p, ":%s NOTICE %s :You cannot kill a virtual admin", MENAME(source_p), source_p->name);
                        return;
                    }
                }
            } else {
            ...
            sendto_one(source_p, ":%s NOTICE %s :You arent in a virtual server!", MENAME(source_p), source_p->name);
            return;
        }
    }
    ...
    sendto_realops_flags(UMODE_ALL, L_ALL,
                 ...
                 target_p->name, source_p->name, MENAME(source_p), reason);

    ilog(L_INFO, "KILL From %s For %s Path %s (%s)",
         source_p->name, target_p->name, MENAME(source_p), reason);
    log_oper_action(LOG_KILL_TYPE, source_p, "%s %s\n", MENAME(source_p), reason);
...
ms_kill(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
    ...
    if(!IsAnVOper(source_p) && !IsServer(source_p) && !IsServices(source_p) && !IsOper(source_p)) {
        sendto_one(source_p, form_str(ERR_NOPRIVILEGES), MENAME(source_p), source_p->name);
        return;
    }
    ...
    if(IsAnVOper(source_p)) {
        if(source_p->vlink_nome) {
            if(target_p->vlink_nome) {
                if(!match(source_p->vlink_nome, target_p->vlink_nome)) {
                    sendto_one(source_p, ":%s NOTICE %s :Nick %s isnt on your server", MENAME(source_p), source_p->name, target_p->name);
                    return;
                } else {
                    if(IsVAdmin(target_p) && target_p != source_p) {
                        sendto_one(source_p, ":%s NOTICE %s :You cannot kill a virtual admin", MENAME(source_p), source_p->name);
