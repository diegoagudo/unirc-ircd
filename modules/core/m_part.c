...
part_one_client(struct Client *client_p, struct Client *source_p, char *name, char *reason)
{
	...
	{
		sendto_one(source_p, form_str(ERR_NOSUCHCHANNEL), MENAME(source_p), source_p->name, name);
		return;
	}

	if((ms = find_channel_link(source_p, chptr)) == NULL)
	{
		sendto_one(source_p, form_str(ERR_NOTONCHANNEL), MENAME(source_p), source_p->name, name);
		return;
	}
...
m_part(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	...
	if(*parv[1] == '\0')
	{
		sendto_one(source_p, form_str(ERR_NEEDMOREPARAMS), MENAME(source_p), source_p->name, "PART");
		return;
	}
