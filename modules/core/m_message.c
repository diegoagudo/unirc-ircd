	  ...
	  struct Client *source_p, int parc, char *parv[])
{
    ...
    if(!IsServer(source_p) && !IsServices(source_p) && (!IsOper(source_p) && MyClient(source_p)))
    {
      if(parv[2])
      {
          if(verifica_spamsg(parv[2])) { //by XOOM <xoom@unirc.net>
              //sendto_one(source_p,":%s NOTICE %s :Esta mensagem n�o pode ser enviada, pois cont�m palavras consideradas proibidas na UnIRC.", me.name, parv[0]);
              sendto_ircops("SPAM de \2%s!%s@%s\2 para \2%s\2 - Mensagem \2%s\2", 
                   source_p->name, source_p->username, source_p->realhost, parv[1], parv[2]);
           return;
          }
      }
    }
	    ...
	    struct Client *source_p, struct Channel *chptr, char *text)
{
	...
	if((result = can_send(chptr, source_p, NULL, text, p_or_n)) > 0)
	{
		...
		{
           if (MyClient(source_p) && (chptr->mode.mode & MODE_STRIPCOLOR)) 
                sendto_channel_butone(client_p, source_p, chptr, command, ":%s", StripControlCodes(text));
           else
		...
		if(p_or_n != NOTICE)
		{
			if(result == CAN_SEND_NOREGNICK)
				...
				sendto_one(source_p, form_str(ERR_CANNOTSENDTOCHAN),
					   ID_or_name(&me, client_p),
					   ID_or_name(source_p, client_p), chptr->chname, "(O canal não aceita propagandas)");
			else if(result == -5)
				sendto_one(source_p, form_str(ERR_CANNOTSENDTOCHAN),
					   ID_or_name(&me, client_p),
					   ID_or_name(source_p, client_p), chptr->chname, "(Impossível mandar mensagens repetidas para o canal)");
			else if(result == -6) {
				static char comment[1024];
				ircsprintf(comment, "Flood! (%i lines in %i seconds)",
