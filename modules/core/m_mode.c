...
m_mode(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	...
	if(*parv[1] == '\0')
	{
		sendto_one(source_p, form_str(ERR_NEEDMOREPARAMS), MENAME(source_p), source_p->name, "MODE");
		return;
	}
	...
	{
		sendto_one(source_p, form_str(ERR_BADCHANNAME), MENAME(source_p), source_p->name, parv[1]);
		return;
	}
	...
	if(parc < 3)
	{
		...
		sendto_one(source_p, form_str(RPL_CHANNELMODEIS),
			   MENAME(source_p), source_p->name, chptr->chname, modebuf, parabuf);
		sendto_one(source_p, form_str(RPL_CREATIONTIME),
			   MENAME(source_p), source_p->name, chptr->chname, chptr->channelts);
	}
