...
static void perform_nick_collides(struct Client *, struct Client *, struct Client *,
				  int, char **, time_t, char *, char *, char *, char *);
...
m_nick(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
    ...
    // uhauha cara de zombie nao vai muda o nick!! :X uhauhahau
    if(IsZombie(source_p))
        return;
...
ms_nick(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	...
	if(parc == 12)
	{
		...
		strlcpy(nvlink, parv[10], sizeof(nvlink));
		strlcpy(ngecos, parv[11], sizeof(ngecos));
	...
	if(!(target_p = find_client(nick)))
	{
		nick_from_server(client_p, source_p, parc, parv, newts, nick, nvlink, ngecos);
		return;
	}
	...
	perform_nick_collides(source_p, client_p, target_p, parc, parv, newts, nick, nvlink, ngecos, NULL);
}
...
ms_uid(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	...
	/* Fix the lengths */
	strlcpy(nick, parv[1], sizeof(nick));
	strlcpy(uvlink, parv[11], sizeof(uvlink));
	...
	if((target_p = find_client(unick)) == NULL)
		client_from_server(client_p, source_p, parc, parv, newts, nick, uvlink, ugecos);
	else if(IsUnknown(target_p))
	{
		...
		perform_nick_collides(source_p, client_p, target_p,
				      parc, parv, newts, nick, uvlink, ugecos, uid);
}
...
nick_from_server(struct Client *client_p, struct Client *source_p, int parc,
		 char *parv[], time_t newts, char *nick, char *nvlink, char *ngecos)
{
	if(IsServer(source_p))
	{
		...
		if(parc > 10)
		{
			...
			register_remote_user(client_p, source_p, parv[5], parv[6], parv[7], nvlink, ngecos,
					     parv[9]);
			return;
		}
	}
...
client_from_server(struct Client *client_p, struct Client *source_p, int parc,
		   char *parv[], time_t newts, char *nick, char *uvlink, char *ugecos)
{
	char *m;
	unsigned int flag;
	...
	register_remote_user(client_p, source_p, parv[5], parv[6], servername, uvlink, ugecos, parv[10]);
}

static void
perform_nick_collides(struct Client *source_p, struct Client *client_p,
		      struct Client *target_p, int parc, char *parv[],
		      time_t newts, char *nick, char *vlink, char *gecos, char *uid)
{
	int sameuser;
	 ...
	 */
	nick_from_server(client_p, source_p, parc, parv, newts, nick, vlink, gecos);
}
