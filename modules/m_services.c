...
static void m_ircops(struct Client *, struct Client *, int, char *[]);
...
static void m_zombies(struct Client *, struct Client *, int, char *[]);
...
/* m_ircops */
struct Message ircops_msgtab = {
	"IRCOPS", 0, 0, 0, 0, MFLG_SLOW, 0,
	{m_unregistered, m_ircops, m_ircops, m_ignore, m_ircops, m_ignore}
};

struct Message zombies_msgtab = {
	"ZOMBIES", 0, 0, 0, 0, MFLG_SLOW, 0,
	{m_unregistered, m_not_oper, m_zombies, m_ignore, m_zombies, m_ignore}
};
...
_modinit(void)
{
	...
	mod_add_cmd(&ircops_msgtab);
	mod_add_cmd(&zombies_msgtab);
}
...
_moddeinit(void)
{
	...
	mod_del_cmd(&ircops_msgtab);
	mod_del_cmd(&zombies_msgtab);
}
    ...
    for(p = &parv[3]; p && *p; p++)
    {
        for(m = *p; *m; m++)
        {
            ...
            case 'x':
                if(what == MODE_ADD)
                {
                  // agora essa merlim volta criptada qdo SVSMODE usado pelo services :P
                  // by -XOOM
        					...
        					make_virthost(target_p->sockhost, target_p->host);
            ...
            case 'B' :
              if ((what == MODE_ADD) && (!IsRegNick(target_p)) ) {
                      //sendto_one(target_p, ":%s NOTICE %s :\24ATEN��O:\2 Voc� n�o poder� enviar mensagens ou entrar em canais enquanto n�o identificar seu nick ou registra-lo. Para mais informa��es, por favor digite \002/NICKSERV HELP\002", me.name, target_p->name);
                      do_join_0(client_p, target_p);
                      SetGringo(target_p);
              } else ClearGringo(target_p);
              break;

            case 'r' :
                 if (what == MODE_ADD) {
                     if (IsGringo(target_p))
                         ClearGringo(target_p);
                     SetRegNick(target_p);
                 } else ClearRegNick(target_p);
                 break;
...
static void
m_ircops(struct Client* client_p, struct Client* source_p, int parc, char* parv[])
{
  struct Client* target_p;
  dlink_node *gcptr;
  ...
  DLINK_FOREACH_SAFE(gcptr, gcptr_next, global_client_list.head)
  {
    target_p = gcptr->data;
         ...
         if ( (IsOper(target_p)) && !IsServices(target_p) )
         {
           ...
           ircsprintf(buf, "%-19s  %-14s  %-5s  %s", target_p->name ? target_p->name : "<unknown>", status,
                                       target_p->away ? "(AWAY)" : "", target_p->servptr->name); 

                                                                                                      
                  sendto_one(client_p, form_str(RPL_MAP), me.name, client_p->name, buf);
                  if (IsAdmin(target_p)) globals++;
                  else locals++;
         }
         if(source_p->vlink_nome) {
             if(target_p->vlink_nome) {
                 if(match(source_p->vlink_nome,target_p->vlink_nome)) {
                     if ( (IsAnVOper(target_p)) )
                     {
                          
                        if (IsVAdmin(target_p)) 
                            status = "IRCAdmin";
                        else if(IsVOper(target_p))
                            status = "IRCop"; 
                        else
                            status = "IRCop";

                       ircsprintf(buf, "%-19s  %-14s  %-5s  %s", target_p->name ? target_p->name : "<unknown>", status,
                                                   target_p->away ? "(AWAY)" : "", target_p->vlink_nome); 

                                                                                                                  
                              sendto_one(client_p, form_str(RPL_MAP), me.name, client_p->name, buf);
                              if (IsVAdmin(target_p)) globals++;
                              else locals++;
                     }
                 }
             }
  ...
  ircsprintf(buf, "Total: \2%d\2 IRCop%s online - \2%d\2 Global%s, \2%d\2 Local%s", globals+locals,
                        (globals+locals) > 1 ? "s" : "", 
                        globals, globals > 1 ? "s" : "", locals, locals > 1 ? "s" : "");
  sendto_one(client_p, form_str(RPL_MAP), me.name, client_p->name, buf);
  sendto_one(client_p, form_str(RPL_MAPEND), me.name, client_p->name, "IRCOPS");
  return;
}


static void
m_zombies(struct Client* client_p, struct Client* source_p, int parc, char* parv[])
{
  struct Client* target_p;
  dlink_node *gcptr;
  dlink_node *gcptr_next;  
  ...
  strcpy(buf, "\2Nick          Host          Status          Server\2");
  sendto_one(client_p, form_str(RPL_MAP), me.name, client_p->name, buf);;
  strcpy(buf, "--------------------------------------------------------");
  sendto_one(client_p, form_str(RPL_MAP), me.name, client_p->name, buf);
  ...
  DLINK_FOREACH_SAFE(gcptr, gcptr_next, global_client_list.head)
  {
    target_p = gcptr->data;
     ...
     if ((IsGringo(target_p) || IsZombie(target_p)) && !IsServices(target_p) )
     {
          
       if (IsZombie(target_p)) 
        status = "Zombie";
       else if(IsGringo(target_p))
        status = "Gringo"; 
       else
        status = "";

       ircsprintf(buf, "%-12s  %-12s %-12s  %s", target_p->name ? target_p->name : "<unknown>", 
                                                target_p->sockhost, 
                                                status,
              ...
              sendto_one(client_p, form_str(RPL_MAP), me.name, client_p->name, buf);
              if (IsZombie(target_p)) globals++;
              else locals++;
     }
  }
  
  ircsprintf(buf, "Total: \2%d\2 Zombie%s conectado%s - \2%d\2 Zombie%s, \2%d\2 Gringo%s", globals+locals,
                        (globals+locals) > 1 ? "s" : "", (globals+locals) > 1 ? "s" : "",
                        globals, globals > 1 ? "s" : "", locals, locals > 1 ? "s" : "");
  sendto_one(client_p, form_str(RPL_MAP), me.name, client_p->name, buf);
  sendto_one(client_p, form_str(RPL_MAPEND), me.name, client_p->name, "ZOMBIES");
  return;
}
