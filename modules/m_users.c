...
m_users(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	...
	if(!ConfigFileEntry.disable_remote)
		if(hunt_server(client_p, source_p, ":%s USERS :%s", 1, parc, parv) != HUNTED_ISME)
			return;
/*
	sendto_one(source_p, form_str(RPL_LOCALUSERS2), me.name, source_p->name,
...
*/
	sendto_one(source_p, form_str(RPL_LOCALUSERS2), me.name, source_p->name,
		   ConfigServerHide.hide_servers ? Count.total : Count.total,
		   ConfigServerHide.hide_servers ? Count.max_tot : Count.max_tot,
		   ConfigServerHide.hide_servers ? Count.total : Count.total,
		   ConfigServerHide.hide_servers ? Count.max_tot : Count.max_tot);

	sendto_one(source_p, form_str(RPL_GLOBALUSERS2), me.name, source_p->name,
