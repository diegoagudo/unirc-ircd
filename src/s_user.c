...
show_lusers(struct Client *source_p)
{
	...
	if(!ConfigServerHide.hide_servers || IsOper(source_p))
	{
		...
		sendto_one(source_p, form_str(RPL_LOCALUSERS2),
	...
	sendto_one(source_p, form_str(RPL_GLOBALUSERS2),
		   from, to, Count.total, Count.max_tot, Count.total, Count.max_tot);
...
show_lusers(struct Client *source_p)
{
    ...
    {
        from = MENAME(source_p);
        to = source_p->name;
    }
    
    sendto_one(source_p, form_str(RPL_LUSERCLIENT),  from, to, (Count.total - Count.invisi), Count.invisi, (dlink_list_length(&global_serv_list) + MaxVlink + 1));
    
    if(Count.oper > 0)
        sendto_one(source_p, form_str(RPL_LUSEROP),      from, to, Count.oper);
    
    if(dlink_list_length(&unknown_list) > 0)
        sendto_one(source_p, form_str(RPL_LUSERUNKNOWN), from, to, dlink_list_length(&unknown_list));
    
    if(dlink_list_length(&global_channel_list) > 0)
        sendto_one(source_p, form_str(RPL_LUSERCHANNELS),from, to, dlink_list_length(&global_channel_list));
    
    sendto_one(source_p, form_str(RPL_LUSERME),      from, to, Count.total, (dlink_list_length(&global_serv_list) + MaxVlink + 1));
    
    sendto_one(source_p, form_str(RPL_LOCALUSERS2),  from, to, Count.total, Count.max_tot, Count.total, Count.max_tot);
    sendto_one(source_p, form_str(RPL_GLOBALUSERS2), from, to, Count.total, Count.max_tot, Count.total, Count.max_tot);
		    ...
		    const char *nick, const char *username)
{
	...
	if(valid_hostname(source_p->host) == 0)
	{
		sendto_one(source_p, ":%s NOTICE %s :*** Notice -- You have an illegal "
			   "character in your hostname", MENAME(source_p), source_p->name);
	...
	if(!IsGotId(source_p))
	{
		...
		if(IsNeedIdentd(aconf))
		{
			...
			sendto_one(source_p, ":%s NOTICE %s :*** Notice -- You need to install "
				   "identd to use this server", MENAME(source_p), source_p->name);
	...
	if(!EmptyString(aconf->passwd))
	{
		...
		{
			ServerStats->is_ref++;
			sendto_one(source_p, form_str(ERR_PASSWDMISMATCH), MENAME(source_p), source_p->name);
    ...
    */
    ...
    sendto_ircops_client_connect("Client connecting: %s (%s@%s) [%s] {%s}",
			     nick, source_p->username, source_p->realhost,
			     ConfigFileEntry.hide_spoof_ips && IsIPSpoof(source_p) ?
			     "255.255.255.255" : ipaddr, get_client_class(source_p));
	...
	{
		make_virthost(source_p->sockhost, source_p->host);
...
register_remote_user(struct Client *client_p, struct Client *source_p,
		     ...
		     const char *uvlink, const char *realname, const char *realhost)
{
    ...
    if ((vlink = find_vlink_nome(uvlink))) {
         if (vlink->server_filho) DupString(source_p->vlink_nome, vlink->server_filho);
         if (vlink->descricao) DupString(source_p->vlink_desc, vlink->descricao);
    } else {
        source_p->vlink_nome = NULL;
        source_p->vlink_desc = NULL;
    }
...
introduce_client(struct Client *client_p, struct Client *source_p)
{
    ...
    if(!ServerInfo.hub && uplink && IsCapable(uplink, CAP_LL) && client_p != uplink)
    {
        ...
        {
            sendto_one(uplink, ":%s UID %s %d %lu %s %s %s %s %s %lu %s %s :%s",
                   source_p->servptr->id,
                   ...
                   source_p->realhost, source_p->vlink_nome ? source_p->vlink_nome : me.name, source_p->info);
        }
        else
        {
            sendto_one(uplink, "NICK %s %d %lu %s %s %s %s %lu %s %s :%s",
                   source_p->name, source_p->hopcount + 1,
                   (unsigned long) source_p->tsinfo,
                   ubuf, source_p->username, source_p->host,
    ...
    else
    {
        DLINK_FOREACH(server_node, serv_list.head)
        {
            ...
            if(IsCapable(server, CAP_TS6) && HasID(source_p)) {
                sendto_one(server, ":%s UID %s %d %lu %s %s %s %s %s %lu %s %s :%s",
                       source_p->servptr->id,
                       source_p->name, source_p->hopcount + 1,
                       (unsigned long) source_p->tsinfo,
                       ubuf, source_p->username, source_p->host,
                ...
                sendto_one(server, "NICK %s %d %lu %s %s %s %s %lu %s %s :%s",
                       source_p->name, source_p->hopcount + 1,
                       (unsigned long) source_p->tsinfo,
                       ubuf, source_p->username, source_p->host,
	      ...
	      const char *username, const char *host, const char *server, const char *realname)
{
    ...
    if (server[0] == '"' && server[1]) {
         int len = (strlen(server) - 2);
         strlcpy(s1, server+1, sizeof(s1));
         s1[len] = '\0';
    } else {
        strlcpy(s1, server, sizeof(s1));
        s1[HOSTLEN] = '\0';
    }
    
    //sendto_one(source_p, "NOTICE %s : End: %s",source_p->name, s1);
    
    if ((vlink = find_vlink(s1))) {
         if (vlink->server_filho) DupString(source_p->vlink_nome, vlink->server_filho);
         if (vlink->descricao) DupString(source_p->vlink_desc, vlink->descricao);
        
        //sendto_one(source_p, "NOTICE %s : Virtual: %s Desc: %s", source_p->name, client_p->vlink_nome, client_p->vlink_desc);
    } else {
        source_p->vlink_nome = NULL;
        source_p->vlink_desc = NULL;
    }
...
set_user_mode(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	...
	for(p = &parv[2]; p && *p; p++)
	{
      ...
      case 'p':
      		if(what == MODE_ADD)
            SetHideChannels(source_p);
          else
            ClearHideChannels(source_p);
      break;

			case 'x':
			 // agora so Services && Servidor mexem no HIDDEN do user! :p
			 // by -XOOM.
			 if(IsServices(client_p) && IsServer(client_p))
			 {
    				if(what == MODE_ADD)
    				{
			...
			case 'r':  // nick registrado
			case 'Z':  // zombie
			case 'B':  // gringo
			case 'N':  // netadmin(services root)
			case 'a':  // services admin
			//case 'A':  // ircadmin
			case 'O':  // services oper
			case 'h':  // helper
			case 'S':  // sll
			case 'V':  // VADMIN
			case 'v':  // VOPER
...
static void
verifica_gringo(struct Client *source_p)
{
  if(!(find_proxy(source_p->sockhost)) && !(find_proxy(source_p->realhost)))
  {
      if (!(find_ipbr(source_p->sockhost))) {
          sendto_one(source_p, ":%s NOTICE %s :You won't be able to send messages or join a channel while your nickname is not identified or registered. To register your nickname, type \002/msg NICKSERV REGISTER PASS MAIL\002 or type \002/join #Help\002 for help", MENAME(source_p), source_p->name);
          source_p->umodes |= UMODE_GRINGO;
      }
    }
}
...
user_welcome(struct Client *source_p)
{
	...
	if(IsSSL(source_p))
		sendto_one(source_p, ":%s NOTICE %s :*** Connected securely via %s",
			   MENAME(source_p), source_p->name, ssl_get_cipher(source_p->localClient->fd.ssl));
#endif

/*	if(IsCloaked(source_p))
		sendto_one(source_p, ":%s NOTICE %s :*** Seu host est� encriptado (%s)", MENAME(source_p),
			   source_p->name, source_p->host);*/

	sendto_one(source_p, form_str(RPL_WELCOME), MENAME(source_p), source_p->name,
		   ServerInfo.network_name, source_p->name, source_p->username, source_p->sockhost);

	//sendto_one(source_p, form_str(RPL_YOURHOST), MENAME(source_p), source_p->name,
	//	   get_listener_name(source_p->localClient->listener), ircd_version);

	sendto_one(source_p, form_str(RPL_YOURHOST), MENAME(source_p), source_p->name,
		   MENAME(source_p), ircd_version);

	sendto_one(source_p, form_str(RPL_CREATED), MENAME(source_p), source_p->name, built_date);
	sendto_one(source_p, form_str(RPL_MYINFO),
		   MENAME(source_p), source_p->name, MENAME(source_p), ircd_version, umode_buffer);
	show_isupport(source_p);

	if(source_p->id[0] != '\0')
		sendto_one(source_p, form_str(RPL_YOURID), MENAME(source_p), source_p->name, source_p->id);

	show_lusers(source_p);

    //sendto_one(source_p, ":%s NOTICE %s :*** Notice -- MOTD was last changed at %s", MENAME(source_p),
	...
	if(ConfigFileEntry.short_motd)
	{
		...
		sendto_one(source_p, form_str(RPL_MOTDSTART), MENAME(source_p), source_p->name, MENAME(source_p));
		sendto_one(source_p, form_str(RPL_MOTD),
			   MENAME(source_p), source_p->name, "*** This is the short motd ***");
		sendto_one(source_p, form_str(RPL_ENDOFMOTD), MENAME(source_p), source_p->name);
	}
	else
		send_message_file(source_p, &ConfigFileEntry.motd);
		

		verifica_gringo(source_p);
}
...
oper_up(struct Client *source_p)
{
  ...
  if(IsAdmin(source_p)) {
	   sendto_realops_flags(UMODE_ALL, L_ALL, "%s (%s@%s) is now a Server Administrator (A)",
		               	      source_p->name, source_p->username, source_p->host);
  } else {
	   sendto_realops_flags(UMODE_ALL, L_ALL, "%s (%s@%s) is now an IRC Operator (o)",
		               	      source_p->name, source_p->username, source_p->host);
  }
