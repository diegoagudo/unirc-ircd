...
sendnick_TS(struct Client *client_p, struct Client *target_p)
{
    ...
    if(HasID(target_p) && IsCapable(client_p, CAP_TS6))
        sendto_one(client_p, ":%s UID %s %d %lu %s %s %s %s %s %lu %s %s :%s",
               target_p->servptr->id,
               ...
               (unsigned long) target_p->servicestamp, target_p->realhost,
                target_p->vlink_nome ? target_p->vlink_nome : me.name, target_p->info);
    else
        sendto_one(client_p, "NICK %s %d %lu %s %s %s %s %lu %s %s :%s",
               target_p->name, target_p->hopcount + 1,
               (unsigned long) target_p->tsinfo,
               ubuf, target_p->username, target_p->host,
               target_p->servptr->name,
               (unsigned long) target_p->servicestamp,
               target_p->realhost, target_p->vlink_nome ? target_p->vlink_nome : me.name, target_p->info);
...
server_estab(struct Client *client_p)
{
    ...
    SEND_PROXYS(client_p);
    SEND_IPBRS(client_p);
    SEND_SPAMSGS(client_p);
    SEND_VLINK(client_p);
