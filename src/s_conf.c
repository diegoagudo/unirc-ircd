...
check_client(va_list args)
{
    ...
    case TOO_MANY:
        sendto_one(source_p, ":%s NOTICE %s :*** IP: %s [%s] - Clones are forbidden on the UnIRC. If you need PROXY to connect on our network or to run allowed bots, join #Help and ask about.", MENAME(source_p), source_p->name, source_p->sockhost, source_p->realhost);
	...
	if(aconf != NULL)
	{
			...
			|| (ConfigFileEntry.glines && IsConfGline(aconf)))
		{
			...
			if(ConfigFileEntry.kline_with_reason)
                sendto_one(client_p, ":%s NOTICE %s :*** IP: %s [%s] - You're banned from UnIRC. Reason %s - If you think it was a mistake, send us e-mail admin@unirc.net", MENAME(client_p), client_p->name, client_p->sockhost, client_p->realhost, aconf->reason);
...
attach_iline(struct Client *client_p, struct ConfItem *conf)
{
    ...
    if(!(find_proxy(client_p->sockhost)) && !(find_proxy(client_p->realhost)))
    {
