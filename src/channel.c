...
static  int check_for_chan_flood(struct Client *, struct Channel *, struct Membership *);
 ...
 * @return
 */
static int check_for_chan_flood(struct Client *cptr, struct Channel  *chptr, struct Membership *lp)
{
	...
	if ( (CurrentTime - (lp->lmsg_time))  >= /* current - lastmsgtime */
