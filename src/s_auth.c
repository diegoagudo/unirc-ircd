...
static const char *HeaderMessages[] = {
	":%s NOTICE AUTH :*** Procurando seu hostname...",
	":%s NOTICE AUTH :*** Hostname encontrado",
	":%s NOTICE AUTH :*** Não foi possível encontrar seu hostname, usando o IP numérico",
	":%s NOTICE AUTH :*** Checando ident",
	":%s NOTICE AUTH :*** Resposta do ident recebida",
	":%s NOTICE AUTH :*** Nenhuma resposta do ident; username prefixado com ~ (til)",
...
#define sendheader(c, i) sendto_one((c), HeaderMessages[(i)])
...
start_auth(va_list args)
{
	...
	sendheader(client, REPORT_DO_ID);  
	sendheader(client, REPORT_FIN_ID);
