...
sendto_common_channels_local(struct Client *user, int touser, const char *pattern, ...)
{
	...
	DLINK_FOREACH(cptr, user->channel.head)
	{
		...
		DLINK_FOREACH(uptr, chptr->members.head)
		{
            ...
            //if ((chptr->mode.mode & MODE_NOQUIT) && (user->flags & FLAGS_NORMALEX))
              //  len = send_format(buffer, IRCD_BUFSIZE, "Quit: ", args);
 ...
 */
void
sendto_ircops(const char *pattern, ...)
{
    struct Client *target_p;
    char nbuf[IRCD_BUFSIZE];
    dlink_node *gcptr;
    ...
    DLINK_FOREACH_SAFE(gcptr, gcptr_next, global_client_list.head)
    {
        ...
        if (IsNetAdmin(target_p)) {
            sendto_one(target_p, ":%s NOTICE %s :*** %s", me.name, target_p->name, nbuf);
...
void
sendto_ircops_client_connect(const char *pattern, ...)
{
    struct Client *target_p;
    char nbuf[IRCD_BUFSIZE];
    dlink_node *gcptr;
    ...
    DLINK_FOREACH_SAFE(gcptr, gcptr_next, global_client_list.head)
    {
        ...
        if (IsConn(target_p)) {
            sendto_one(target_p, ":%s NOTICE %s :*** Notice -- %s", me.name, target_p->name, nbuf);
        }
    }
}
